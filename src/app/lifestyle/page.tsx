...
export default function LifestylePage() {
  ...
  const fetchNews = async () => {
    try {
      console.log('Fetching lifestyle news...')
      ...
      if (error) {
        ...
        console.log('Lifestyle news fetched:', data)
        setNews(data || [])
        setError(null)
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-12">
        <div className="text-xl">⏳ लाइफ-स्टाइल की खबरें लोड हो रही हैं...</div>
      </div>
    )
  }
  ...
  if (news.length === 0) {
    return (
      <div className="text-center py-12">
        <h1 className="text-4xl font-bold text-gray-800 mb-4">🌸 लाइफ-स्टाइल</h1>
        ...
        {news.map((article) => (
          <article key={article.id} className="bg-white shadow-lg rounded-lg overflow-hidden border border-gray-200 hover:shadow-xl transition-shadow">
            ...
            <div className="p-4 pb-2">
              <span className="inline-block px-3 py-1 rounded-full text-sm font-semibold bg-pink-100 text-pink-800">
                लाइफ-स्टाइल
              </span>
            </div>
