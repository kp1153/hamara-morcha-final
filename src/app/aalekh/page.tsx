...
export default function AalekhPage() {
  ...
  const fetchNews = async () => {
    try {
      console.log('Fetching aalekh news...')
      ...
      if (error) {
        ...
        console.log('Aalekh news fetched:', data)
        setNews(data || [])
        setError(null)
      }
    } catch (err) {
      console.error('Error fetching aalekh news:', err)
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-12">
        <div className="text-xl">⏳ आलेख लोड हो रहे हैं...</div>
      </div>
    )
  }
  ...
  if (news.length === 0) {
    return (
      <div className="text-center py-12">
        <h1 className="text-4xl font-bold text-gray-800 mb-4">📝 आलेख</h1>
