...
export default function PratirodhPage() {
  ...
  const fetchNews = async () => {
    try {
      console.log('Fetching pratirodh news...')
      const { data, error } = await supabase
        .from('news_articles')
        .select('*')
        .eq('category', 'pratirodh')
      ...
      if (error) {
        ...
        console.log('Pratirodh news fetched:', data)
        setNews(data || [])
        setError(null)
      }
    } catch (err) {
      console.error('Error fetching pratirodh news:', err)
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-12">
        <div className="text-xl">⏳ प्रतिरोध की खबरें लोड हो रही हैं...</div>
      </div>
    )
  }
  ...
  if (news.length === 0) {
    return (
      <div className="text-center py-12">
        <h1 className="text-4xl font-bold text-gray-800 mb-4">✊ प्रतिरोध</h1>
