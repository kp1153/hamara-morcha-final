...
export default function VividhaPage() {
  ...
  const fetchNews = async () => {
    try {
      console.log('Fetching vividha news...')
      ...
      if (error) {
        ...
        console.log('Vividha news fetched:', data)
        setNews(data || [])
        setError(null)
      }
    } catch (err) {
      console.error('Error fetching vividha news:', err)
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-12">
        <div className="text-xl">⏳ विविध खबरें लोड हो रही हैं...</div>
      </div>
    )
  }
  ...
  if (news.length === 0) {
    return (
      <div className="text-center py-12">
        <h1 className="text-4xl font-bold text-gray-800 mb-4">🌟 विविध</h1>
